...
export function authenticate(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!multiAgent && !config.apiKey) {
    // No agent credentials and no MCP_API_KEY — fail closed
    res.status(401).json({
      jsonrpc: "2.0",
      error: { code: -32001, message: "Unauthorized: No auth configured (set MCP_API_KEY or add email.env to .agents/)" },
      id: null,
    });
    return;
  }
